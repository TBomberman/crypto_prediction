...
def cliffmatrix_write(data, filename, cutoff=None):
    ...
    for idx1 in range(datasize):
        ...
        for idx2 in range(idx1 + 1, datasize):
            denominator = 1 - sum(data[idx1, :] * data[idx2, :]) / (
            sum(data[idx1, :] ** 2) + sum(data[idx2, :] ** 2) - sum(data[idx1, :] * data[idx2, :]))
            if denominator != 0:
...
def cliff_value(input):
    ...
    idx2 = 1
    data, labels = input[0]
    value = round(np.log(abs(labels[idx1] - labels[idx2]) / (1 - np.sum(data[idx1] * data[idx2]) / (
    np.sum(data[idx1] ** 2) + np.sum(data[idx2] ** 2) - np.sum(data[idx1] * data[idx2])))), 3)
    return value
    ...
    for idx1 in range(datasize):
        for idx2 in range(idx1 + 1, datasize):
            ...
            if len(inputvalues) > 1000:
                ...
                jobs = [(input, job_server.submit(cliff_value, (inputvalues,), (), ("math", "numpy",))) for input in
                        inputvalues]
...
def tanimoto(data):
    data = data[:, 0:-1]
    datasize = data.shape[0]
    similvector = [round(abs(sum(data[idx1, :] * data[idx2, :]) / (
    sum(data[idx1, :] ** 2) + sum(data[idx2, :] ** 2) - sum(data[idx1, :] * data[idx2, :]))), 3) for idx1 in
                   range(datasize) for idx2 in range(idx1 + 1, datasize)]
...
def crosscorr(surface, propertysurface1, propertysurface2, minlag, maxlag, step):
    ...
    while lag <= maxlag:
        ...
        for i in range(autolen):
            ...
            for j in idxs:
                key = str(i) + '_' + str(j)
                pairproduct[key] = max(propertysurface1[i] * propertysurface2[j],
                                       propertysurface1[j] * propertysurface2[i])
...
def heatmap(data, labels=None, file=None):
    ...
    fig = figure()
    imshow(data, cmap=cm.jet, interpolation='nearest', alpha=0.85, origin='lower',
...
def scatter2D_plot(x, y, flag, file=None, show=None, outliers=False):
    ...
    plot([min - abs(max) / 10, max + abs(max) / 10], [min - abs(max) / 10, max + abs(max) / 10], 'k',
         label='Perfect fit')
...
def auc_plot_multi(XY, flag="", file="Noname", cutoff=None, show=None, linestyle='rx-.', include_baseline=True,
                   equal_aspect=True):
    """ Method that generates a plot of the ROC curve
        Parameters:
            title: Title of the chart
    ...
    for xy in XY:
        ...
        pylab.plot([x1 for x1 in np.hstack((0, fpr))], [y1 for y1 in np.hstack((0, tpr))], color=color_list[colorid],
                   linewidth=1.0)
        #   pylab.plot([x1 for x1 in precision], [y1 for y1 in recall],color='blue',linewidth=2.0)
        if include_baseline:
            pylab.plot([0.0, 1.0], [0.0, 1.0], 'k--')
...
def scatterdens_plot(x, y, flag, file=None, show=None):
    ...
    from matplotlib.pyplot import setp, getp, scatter, axis, legend, hexbin, plot, xlim, ylim, xlabel, ylabel, title, \
        grid, savefig, figure
...
def surface_vertex_area(surface, connection, property):
    ...
    for idx in range(len_surface):
        prop = np.average(np.hstack(
            (property[connection[idx, 0] - 1], property[connection[idx, 1] - 1], property[connection[idx, 2] - 1])))
        coord = np.vstack(
            (surface[connection[idx, 0] - 1], surface[connection[idx, 1] - 1], surface[connection[idx, 2] - 1]))
...
def cart2sphe(center, vector):
    ...
    if (vector[2] - center[2]) > 0:
        phi = np.arctan(
            (((center[0] - vector[0]) ** 2 + (center[1] - vector[1]) ** 2) ** 0.5) / (center[2] - vector[2]))
...
def test():
    ...
    data, varnames, dataid = read_descriptors(
        '/home/cmse/fer19x/data/csiro/dataset/csv/nanographene-dataset_features_topologicalindex_qsar_IP.csv')
...
def generate_bond_table(coords):
    return np.array([[i, j, 's'] for i in range(coords.shape[0]) for j in range(i, coords.shape[0] - 1) if
                     calc_dist(coords[i,], coords[j,]) < 1.6])
